...
//------------------------------------------------------------------------------------------
	...
	if($field['type'] == T_TEXT_LINE && isset($field['render_as_url']) && $field['render_as_url'] === true) {
		...
		$filename = $val;
		$val = "<div><a href='". get_file_url($filename, $field) ."'>" . $highlighter->highlight(html($val)). '</a></div>';
		if($cur_mode == MODE_VIEW && isset($field['thumbnail'])) {
			$val .= render_thumbnail($filename, $record, $field);
		}
	}
...
//------------------------------------------------------------------------------------------
	$caption = '';
	if(isset($field['thumbnail']['caption_field'])) {
		$caption = safestring($record[$field['thumbnail']['caption_field']]);
	}
	$html = sprintf(
		"<div><a href='%s' title='%s' data-fancybox data-caption='%s'><img class='file-thumb' style='max-width:%s;max-height:%s' src='%s'></img></a></div>",
		get_file_url($filename, $field),
		html($caption),
		html($caption),
		isset($field['thumbnail']['max_width']) ? $field['thumbnail']['max_width'] : '300px',
		isset($field['thumbnail']['max_height']) ? $field['thumbnail']['max_height'] : '300px',
		get_file_url($filename, $field)
	);

	if($render_filename) {
		$html .= sprintf('<div class="help-block">%s</div>', html($filename));
	}
